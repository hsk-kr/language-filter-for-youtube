...
```
home feed DOM ──MutationObserver (debounced 300ms)──▶ scan()
  └─ for each unchecked ytd-rich-item-renderer:
       extractTitle() ─▶ detectByScript() ──match──▶ filterTile()
                          └─ null + AI language selected ─▶ detectWithAi()
filterTile():
  mode "hide"           → display:none
  mode "not-interested" → action queue → open ⋮ menu → click "Not interested"
...
| `content/feed.ts` | YouTube selectors: tiles, titles, ⋮ buttons | yes | no |
| `content/notInterested.ts` | Menu automation with localized label matching | yes | no |
...
- **"Not interested" over hiding** — hiding is cosmetic; the menu click is
  what feeds YouTube's recommendation signal. Hide remains as a mode and as
  the automatic fallback when the menu can't be driven.
...
- **Detection input is the title only** — channel names are often romanized
  and video descriptions aren't in the feed DOM. Titles are the strongest
  signal available without extra requests.
